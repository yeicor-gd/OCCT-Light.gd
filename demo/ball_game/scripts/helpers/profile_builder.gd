...
static func build_profiles(
	...
	assert(wall_height >= 0.0, var_to_str(get_stack()))
	assert(wall_height <= 2.0) # >= 1.0 for roof
	# --- Derived dimensions ---
	...
	var wh := wh_base * wth # wall height above pathway floor
	var wr := wh_base >= 1.0 # With extra roof sketch
	var wh_clamped := minf(wh_base, 1.0) * wth # Clamped wall height for actual wall
	...
	wb.move_to(-pwh, -br + wh_clamped - rt) # Top-left of the inside (for easier core mode)
	if not fancy: # Fast square bottom
		wb.line_to(-pwh, -br)
		wb.line_to(pwh, -br)
		wb.line_to(pwh, -br + wh_clamped)
	else: 
