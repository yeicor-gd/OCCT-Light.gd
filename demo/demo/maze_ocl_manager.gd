...
func _generate():
	...
	status = OclTopoBuild.topo_remove(graph, spline_wire_id.bits)
	assert(status == OclCore.OK, "Got status " + str(OclCore.status_to_string(status)))
	status = OclTopoBuild.topo_remove(graph, plane_face_id.bits)
	assert(status == OclCore.OK, "Got status " + str(OclCore.status_to_string(status)))
	
	print("[OclManager] Generated solid in ", (Time.get_ticks_usec() - start_time) / 1000.0, " ms")
	start_time = Time.get_ticks_usec()
	
	status = OclMeshToGodot.mesh_vertices(graph, $Vertices.multimesh)
	assert(status == OclCore.OK, "Got status " + str(OclCore.status_to_string(status)))
	print("[OclManager] Meshed ", $Vertices.multimesh.instance_count, " vertices in ", (Time.get_ticks_usec() - start_time) / 1000.0, " ms")
	start_time = Time.get_ticks_usec()
	
	status = OclMeshToGodot.mesh_edges(graph, $Edges.multimesh)
	assert(status == OclCore.OK, "Got status " + str(OclCore.status_to_string(status)))
	print("[OclManager] Meshed ", $Edges.multimesh.instance_count, " edge segments in ", (Time.get_ticks_usec() - start_time) / 1000.0, " ms")
	start_time = Time.get_ticks_usec()
